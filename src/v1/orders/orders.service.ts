...
export class OrdersService {
 ...
 async create(createOrderDto:CreateOrderDto,user:any):Promise<{id:number}>{
    return this.prismaService.order.create({
      data:{
        totalAmount:createOrderDto.items.reduce((acc,value)=>acc+=value.price*value.quantity,0),
        user:{
          connect:{
            id:user.id
          },
        ...
        OrderItem:{
          create:createOrderDto.items.map((items)=>({productId:items.productId,quantity:items.quantity,price:items.price}))
        }
      },
      select:{
        id:true,
  ...
  async update(id:number, updateOrderDto:UpdateOrderDto):Promise<{id:number}>{
    return this.prismaService.order.update({where:{id},
      data:{
